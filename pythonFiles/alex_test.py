...
def initialize_cam():
	...
	for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
		...
		ret, mask = cv.threshold(diff, 150, 255, cv.THRESH_BINARY_INV)
		diff = cv.bitwise_and(diff, mask)
		# diff = cv.cvtColor(diff, cv.COLOR_BGR2GRAY)
		diff = cv.cvtColor(diff, cv.COLOR_BGR2HSV)
		diff = cv.inRange(diff, (200, 0, 0), (225, 255, 255))
		...
		retval, threshold = cv.threshold(diff, 1000, 255, cv.THRESH_BINARY)
